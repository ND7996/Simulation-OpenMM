...
prmtop = app.AmberPrmtopFile("GPX6_wat.prmtop")
inpcrd = app.AmberInpcrdFile("GPX6_wat_init0.rst")
...
thermostat = LangevinIntegrator(343*kelvin, 1/picoseconds, 0.001*picoseconds)

# Pressure, Temperature (only used for calculation),
# Frequency (how frequently the system should update the box size)
barostat = MonteCarloBarostat(1.0*atmosphere, 343.0*kelvin, 25)
...
with open("GPX6_chk_out.rst",'rb') as f:
...
datafile=open("GPX6_output1.log","a")
sim.reporters.append(StateDataReporter(datafile,
    2000,   # number of steps between each save.
...
sim.reporters.append(CheckpointReporter('GPX6_chk_out.rst',20000))
...
sim.reporters.append(DCDReporter("GPX6.dcd",
    20000,
    append=True))
